...
async def fetch_page(
    session: aiohttp.ClientSession, limit: int, offset: int
) -> Dict[str, Any]:
    payload = {
        "permalink": PERMALINK,
        "limit": limit,
...
async def save_product_and_specs(
    session_db, source_id: int, product_json: Dict[str, Any]
):
    ...
    sku = (
        product_json.get("id")
        or product_json.get("article")
        or product_json.get("permalink")
        or None
    )
    price = (
        product_json.get("costData")
        or product_json.get("cost")
        or product_json.get("price")
        or None
    )
            ...
            for subk, subv in iter_key_values(v):
                ...
                if isinstance(subv, (dict, list)):
                    ...
                    if num is not None:
                        ...
                        await add_spec(
                            session_db, p.id, spec_name, sval or None, None, None
                        )
                else:
                    sval = str(subv).strip()
                    num, unit = parse_number_and_unit(sval)
    ...
    if image:
        await add_spec(
            session_db, p.id, normalize_spec_name("image"), image, None, None
        )
    if description:
        ...
        await add_spec(
            session_db,
            p.id,
            normalize_spec_name("description"),
            description.strip(),
            None,
            None,
        )
    ...
    async with aiohttp.ClientSession(headers=HEADERS) as session:
        ...
        async with async_session() as db_sess:
            src = await create_source_if_missing(
                db_sess,
                "Comelit Clients Site API",
                "https://comelitrussia.clients.site",
            )
            source_id = src.id
