...
# Надёжный CSS-селектор для карточек
...
def extract_from_anchor(a):
    ...
    name_tag = a.select_one(".product__descr, .product__title, .descr, .title, h3, h4, a[title]")
...
for i, a in enumerate(anchors[:30], 1):
    ...
    print(f"{i:02d}. url={item['url']!r} name={item['name']!r} price={item['price']!r} img={item['image']!r}")